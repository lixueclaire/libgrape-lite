...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_THREE_ACYCLIC_TRIANGLE_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_THREE_ACYCLIC_TRIANGLE_H_
...
class AcyclicTriangleFlash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  void Run(const fragment_t& graph) {
    Print("Run Acyclic Triangle Counting with Flash, ");
    int32_t n_vertex = graph.GetTotalVerticesNum();
    Print("total vertices: %d\n", n_vertex);

		DefineMapV(init) { v.count = 0; v.out.clear(); };
    ...
    GetSum(cnt, cnt_all);
    Print( "number of acyclic triangles=%lld\n", cnt_all);
	}
};
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_ACYCLIC_TRIANGLE_H_
