...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_CUT_POINT_2_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_CUT_POINT_2_H_
...
class CutPoint2Flash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
    ...
    DefineMapV(init) { v.cid = id; v.d = Deg(id); v.dis = -1; v.tmp = 0; };
	  ...
	  for(int len = VSize(A), i = 1; len > 0; len = VSize(A), ++i) {
		...
		DefineMapV(cut_point) {
			v.d = 0;
			int c = (v.p==-1?-1:v.cid);
			for_nb(if(nb.p == id) {if (c==-1) c = nb.cid; else if (nb.cid != c) v.d = 1;})
		};
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_CUT_POINT_2_H_
