...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_BRIDGE_2_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_BRIDGE_2_H_
...
class Bridge2Flash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
    ...
    DefineMapV(init) { v.cid = id; v.d = Deg(id); v.dis = -1; v.tmp = 0; };
	  ...
	  for(int len = VSize(A), i = 1; len > 0; len = VSize(A), ++i) {
		...
		DefineMapV(bridge) {
			v.d = (v.p == -1?0:1);
			for_nb(
				if(v.p==nb_id) {if(v.minp < nb.pre || v.maxp >= nb.pre+nb.nd) v.d=0;}
				else if(id==nb.p) {if(nb.minp < v.pre || nb.maxp >= v.pre+v.nd) v.d=0;}
				else v.d=0;)
		};
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_BRIDGE_2_H_
