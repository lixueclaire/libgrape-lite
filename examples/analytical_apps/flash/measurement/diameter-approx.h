...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_DIAMETER_APPROX_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_DIAMETER_APPROX_H_
...
class DiameterApproxFlash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  void Run(const fragment_t& graph) {
    ...
    DefineMapV(init) { v.ecc = 0; };
      ...
      DefineMapV(local2) { int p = locate(s, id); v.seen|=one<<p; };
      S = VertexMapSeq(S, filter2, local2);
        ...
        DefineMapE(update) { d.seen |= (s.seen & (~d.seen)); d.ecc = std::max(d.ecc, i); };
    ...
    DefineMapV(local) {
      ...
      for (int i = 1; i < k; ++i) 
        if (c[i] < c[p]) 
          p = i; 
      if (v.ecc > c[p].first)
        c[p] = std::make_pair(v.ecc, (int)id);
    };
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_DIAMETER_APPROX_H_
