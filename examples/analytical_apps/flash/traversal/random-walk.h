...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_RANDOM_WALK_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_RANDOM_WALK_H_
...
class RandomWalkFlash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  void Run(const fragment_t& graph) {
    ...
    DefineFV(filter) { return OutDeg(id) > 0; };
    DefineMapV(init_v) {
      vertex_t u;
      u.SetValue(All.fw->Key2Lid(id));
      auto es = graph.GetOutgoingAdjList(u);
      int i = rand() % es.Size();
      vid_t nb_id = graph.Vertex2Gid((es.begin() + i)->get_neighbor());
    ...
    DefineFE(check2) { return OutDeg(sid) > 0; };
	  DefineMapE(update2) { 
      vertex_t u;
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_RANDOM_WALK_H_
