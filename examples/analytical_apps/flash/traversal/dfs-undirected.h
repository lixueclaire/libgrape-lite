...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_DFS_UNDIRECTED_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_DFS_UNDIRECTED_H_
...
class DFSUndirectedFlash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  void Run(const fragment_t& graph) {
    Print("Run DFS-undirected with Flash, ");
    ...
    DefineMapV(init) { v.f = -1; v.deg = Deg(id); };
      ...
      DefineMapV(local) {
        v.pre = v.f;
        for_nb(if(y[nb_id] < x[id] && (v.f == v.pre || deg[nb_id]< deg[v.f])) v.f = nb_id; );
      };
...
#endif  // EXAMPLES_ANALYTICAL_APPS_FLASH_DFS_UNDIRECTED_H_
