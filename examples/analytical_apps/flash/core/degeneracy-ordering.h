...
#ifndef EXAMPLES_ANALYTICAL_APPS_FLASH_DEGENERACY_H_
#define EXAMPLES_ANALYTICAL_APPS_FLASH_DEGENERACY_H_
...
class DegeneracyFlash : public FlashAppBase<FRAG_T, VALUE_T> {
 public:
  ...
  void Run(const fragment_t& graph) {
    Print("Run Degeneracy Ordering with Flash, ");
    ...
    DefineFV(filter) { return v.old != v.core; };
  	...
  	short max_core = 0, dg;
    for (auto &i: All.s)
      max_core = std::max(max_core, GetV(i)->core);
	  GetMax(max_core, dg);
	  Print("degeneracy = %d\n", dg );

	  A = All;
	  for(int len = VSize(A), i = 0; len > 0; len = A.size(), ++i ) {
      ...
      DefineMapV(locald) { v.d -= v.tmp; v.tmp = 0; };
...
#endif  // EXAMPLES_ANALYTICAL_APPS_DEGENERACY_H_
